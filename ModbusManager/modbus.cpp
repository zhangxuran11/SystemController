#include "modbus.h"
#include "ztools.h"
#include<QDebug>

Modbus::Modbus()
{
    regAddr = 0;
    isValid = false;
}
void Modbus::load(QByteArray pkg)
{
    regAddr = 0;
    isValid = false;
    datList.clear();
    char* off = pkg.data();
    addr = (int)off[0];
    code = (int)off[1];
    //qDebug("___addr___ %d",addr);
    //qDebug("___code___ %d",code);

    if(code == 0x03)
    {
        datCount = (int)off[2]/2;
//        qDebug("__datCount__ %d",datCount);
        for(int i = 0;i < datCount;i++)
        {
            int dat = (int)off[3+2*i]<<8;
            dat += (int)off[3+2*i+1];
            datList.append(dat);
        }
        LRC = (quint16)off[pkg.length()-2];
        LRC += (quint16)off[pkg.length()-1] << 8;
//        qDebug("__length__ %d",pkg.length());
//        qDebug("__LRC__ %d",LRC);
    }
    else if(code == 0x10)
    {
        regAddr = (int)off[2] << 8;
        regAddr |= (int)off[3];
        datCount = (int)off[6]/2;
//        qDebug("__datCount__ %d",datCount);
        for(int i = 0;i < datCount;i++)
        {

            int dat = (int)off[7+2*i+1];
            dat += (int)off[7+2*i]<<8;
            datList.append(dat);
        }
        LRC = (quint16)off[pkg.length()-2];
        LRC += (quint16)off[pkg.length()-1] << 8;
//        qDebug("__length__ %d",pkg.length());
//        qDebug("__LRC__ %d",LRC);
    }
    if(datCount != datList.length())
        return;
    rawData = pkg;
    quint16 res = calCLR();
//    qDebug()<<"calCLR= "<<res;
    isValid = LRC == res;

}
typedef unsigned char UCHAR;
typedef unsigned short USHORT;
static const UCHAR aucCRCHi[] = {
    
};
static const UCHAR aucCRCLo[] = {
    
};
USHORT usMBCRC16( UCHAR * pucFrame, USHORT usLen )
{
    UCHAR ucCRCHi = 0xFF;
    UCHAR ucCRCLo = 0xFF;
    int iIndex;
    while( usLen-- )
    {
        iIndex = ucCRCLo ^ *( pucFrame++ );
        ucCRCLo = ( UCHAR )( ucCRCHi ^ aucCRCHi[iIndex] );
        ucCRCHi = aucCRCLo[iIndex];
    }
    return ( USHORT )( ucCRCHi << 8 | ucCRCLo );
}
quint16 Modbus::calCLR()
{
    quint16 sum = 0;
    if(code == 0x10)
    {
        sum = usMBCRC16( (UCHAR*) rawData.data(), datCount*2+7 );
    }
    else if(code == 0x03)
    {
        sum = usMBCRC16( (UCHAR*) rawData.data(), datCount*2+3 );
    }

    return sum;
}

quint16 Modbus::generateCLR()
{
    quint16 sum = 0;
    if(code == 0x10)
    {
        sum = usMBCRC16( (UCHAR*) rawData.data(), 6 );
    }
    else if(code == 0x03)
    {
        sum = usMBCRC16( (UCHAR*) rawData.data(), 6 );
    }
    return sum;
}

void Modbus::generate()
{
    isValid = false;
    rawData.clear();
    //rawData.append(':');
    rawData.append((char*)(&addr),1);
    rawData.append((char*)(&code),1);
    rawData.append(((char*)(&regAddr))+1,1);
    rawData.append(((char*)(&regAddr))+0,1);
    rawData.append(((char*)(&datCount))+1,1);
    rawData.append(((char*)(&datCount))+0,1);
    if(code == 0x10 && false)
    {
        int byteCnt = datCount<<1;
        rawData.append((char*)(&byteCnt),1);
        for(int i = 0;i < datCount;i++)
        {
            int dat = datList[i];
            rawData.append(((char*)(&dat))+1,1);
            rawData.append(((char*)(&dat))+0,1);
        }
    }
    LRC = generateCLR();
    qDebug()<<"LRC = "<<LRC;
    rawData.append(((char*)(&LRC))+0,1);
    rawData.append(((char*)(&LRC))+1,1);
    isValid = true;

}
void Modbus::print()
{
    qDebug()<<"addr     : "<<addr;
    qDebug()<<"code     : "<<code;
    qDebug()<<"regAddr  : "<<regAddr;
    qDebug("DataList :");
    for(int i = 0;i < datList.length();i++)
    {
        qDebug(" %x ",datList[i]);
    }
    qDebug("rawData  :");
    for(int i = 0;i < rawData.length();i++)
    {
        qDebug(" %2x ",(char)rawData[i]);
    }
    qDebug()<<"rawData str:  "<<QString(rawData);
}
